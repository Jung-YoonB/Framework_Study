		...
		try {
			const response = await fetch(`/api/board/${boardId}/comment`, {
				...
				headers: {
					...
					"Content-Type": "application/json"		// 서버로 전달 되는 데이터가 json 임을 알림
				},
...
if(commentList) {
	commentList.addEventListener("click", async function(e) {
		...
		*/
		...
		// closest(선택자) : 클릭한 요소로부터 부모 방향으로 선택자에 해당하는 요소를 찾아주는 메소드
		...
		try{
			const response = await fetch(`/api/comments/${commentId}`, {
				method: "DELETE",	
					// Restful 설계 원칙에 따라 요청 방식은 get, post, put, patch, delete 로 나뉘어짐
