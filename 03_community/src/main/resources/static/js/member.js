...
profileImage.addEventListener('change', function(e) {
	...
	//				base64 라는 문자열로 만들어 주는 객체
	const reader = new FileReader();
	reader.onload = function(e) {
		// 프로필 미리보기 영역에 변환 된 이미지 파일을 표시
		const profilePreview = document.querySelector("#profile-preview");
		profilePreview.src = e.target.result;
		profilePreview.style.display = "block";
		
		// 이미지가 없을 경우 표시 되는 영역은 display:none 변경
		document.querySelector("#profile-preview-placeholder").style.display = "none";
	}
	
	reader.readAsDataURL(file);		// 업로드 한 파일을 base64 방식으로 변경
});
...
checkIdBtn.addEventListener("click", async function() {
	...
	/*
		...
		[encodeURIComponenet()]
		 - 전달하는 파라미터에 '&', '='와 같은 특수 문자가 있을 경우 
		   URL 형식이 깨지는 것을 방지 (인코딩)
		
		["X-Requested-With": "XMLHttpRequest"]
		 - 이 요청이 브라우저 주소창에서 이동한 것이 아니라, 
		   자바스크립트(AJAX)를 통해 보낸 것임을 서버에 알려주는 설정(관례)
	*/
	try {
