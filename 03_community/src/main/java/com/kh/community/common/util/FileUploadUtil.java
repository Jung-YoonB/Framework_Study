...
/*
 * MultipartFile 로 전달 된 파일을 서버 디스크에 저장하는 작업
 * 
 * 	[이미지 업로드 흐름]
 *		1) 브라우저에서 파일을 전송 (form 태그에 enctype="multipart/form-data" 반드시 추가)
 * 		2) 스프링MVC에서 객체로 받아서 컨트롤러로 전달
 * 		3) FileUploadUtil 클래스가 실제 바이트를 특정 폴더(uploads)에 파일로 저장
 * 		4) DB에 파일이 저장 된 경로만 저장
 * 				==> 파일명을 그대로 저장하지 않음
 */
@Component
public class FileUploadUtil {
	/**
	 * 파일 저장 후 경로를 반환하는 메소드
	 * @param file		파일 데이터
	...
	public SavedFile save(MultipartFile file, String uploadDir, String webPrefix) throws IllegalStateException, IOException {
		...
		// lastIndexOf() 마지막 "." 의 인덱스 위치
		int dotIndex = originalName.lastIndexOf(".");
		
		String ext = "";
		// . 에서부터 나머지 index string 떼오기
		// 파일 확장자 없을때 고려해서 dotIndex가 -1 이 아닐 때만...
	...
	/**
	 * 웹 요청 주소와 파일이 저장 된 위치 경로를 전달 받아 해당 파일을 삭제
	 * 
	 * @param webPath 저장 된 웹 요청 주소 (/uploads/profile/xxx.png)
	 * @param uploadDir 파일이 저장 된 위치 (uploads/profile)
	 */
	public boolean delete(String webPath, String uploadDir) {
		if (webPath == null || webPath.isEmpty()) return false;
		if (uploadDir == null || uploadDir.isEmpty()) return false;
